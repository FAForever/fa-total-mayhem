		...
		MainGun = Class(TDFGaussCannonWeapon){
			...
			PlayFxMuzzleSequence = function(self, muzzle)
				...
				for _, v in self.FxGroundEffect do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'BRMT2HT', army, v):ScaleEmitter(1.0))
				end
				for _, v in self.FxVentEffect3 do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'BRMT2HT', army, v):ScaleEmitter(0.7))
				end
				for _, v in self.FxMuzzleEffect do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'Turret_Muzzle', army, v):ScaleEmitter(3.45))
				end
				for _, v in self.FxVentEffect5 do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'Turret_Muzzle01', army, v):ScaleEmitter(2.15))
				end
				for _, v in self.FxVentEffect5 do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'Turret_Muzzle02', army, v):ScaleEmitter(2.15))
				end
				for _, v in self.FxVentEffect do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'vent01', army, v):ScaleEmitter(1.7))
				end
				for _, v in self.FxVentEffect do
					army.TrashBagAdd(trash, CreateAttachedEmitter(unit, 'vent02', army, v):ScaleEmitter(1.7))
				end
